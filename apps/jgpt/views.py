...
def receiveRecord(request, name):
    if name == 'interfaceList':
        json_interfacelist = '{"data":[{"NAME":"1"}],"success":"1"}'
        print('进入测试interfaceList')
        return HttpResponse(json.dumps(json.loads(json_interfacelist)),content_type="application/json")
    print(f'请求名字为{name}')
